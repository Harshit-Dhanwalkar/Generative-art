...
int main() {
    ...
    GIFRecorder gifRecorder(windowWidth, windowHeight, 300);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                ...
                if (event.key.code == sf::Keyboard::Q) {
                    ...
                    std::cout << "Saved image to generated_art.png" << std::endl;
                }
            }
        }
        ...
        for (size_t i = 0; i < shapeTypes.size(); ++i) {
            ...
            if (shapeTypes[i] == 0 || shapeTypes[i] == 2) {
                if (rectIndex < rects.size()) {
                    ...
                    rects[rectIndex].setOrigin(originalRectSizes[rectIndex].x / 2, originalRectSizes[rectIndex].y / 2);
                    
                    // CORRECTED size update: set size based on the ORIGINAL size
                    rects[rectIndex].setSize(sf::Vector2f(originalRectSizes[rectIndex].x * sizeFactor, originalRectSizes[rectIndex].y * sizeFactor));
                ...
                if (circleIndex < circles.size()) {
                    ...
                    circles[circleIndex].setOrigin(originalCircleRadii[circleIndex], originalCircleRadii[circleIndex]);
                    
                    // CORRECTED size update: set radius based on the ORIGINAL radius
                    circles[circleIndex].setRadius(originalCircleRadii[circleIndex] * sizeFactor);
        ...
        if (font.getInfo().family != "") {
            ...
            instructions.setString("R: Start/stop recording | S: Save image | Q: Quit");
            ...
            if (gifRecorder.isRecordingNow()) {
                ...
                recordingText.setPosition(10, 50);
        ...
        window.display();
        gifRecorder.captureFrame(window);
        frameCount++;
    }
